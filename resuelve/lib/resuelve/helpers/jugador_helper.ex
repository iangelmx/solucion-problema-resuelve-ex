defmodule Resuelve.Helpers.JugadorHelper do
  ...
  def get_level_goal_goals(nivel) when nivel != "" and nivel != nil do
    ...
    with %Resuelve.Level{min_goals: min_goals} <- result do
      min_goals
    else
      err -> err
    end
  ...
  def calculate_single_compliance(%Jugador{goles_minimos: goles_min, goles: goles})
      when goles == nil or goles_min == nil do
    nil
  end

  def calculate_single_compliance(%Jugador{goles_minimos: goles_min, goles: goles})
      when goles >= goles_min and goles > 0 do
    100.0
  end
  ...
  def calcula_sueldo_completo(
        jugador = %Jugador{alcance_ind: alcance_ind, alcance_team: alcance_team}
      )
      when alcance_ind != nil and alcance_team != nil do
  ...
  def get_global_compliance(%Jugador{alcance_ind: single_comp, alcance_team: team_comp})
      when single_comp != nil and team_comp != nil do
  ...
  def calculate_bonus_part(global_compliance, %Jugador{bono: bonus})
      when bonus > 0 and bonus != nil and global_compliance > 0 and global_compliance != nil do
  ...
  def calculate_salary(jugador = %Jugador{sueldo: salary, bono: bonus})
      when bonus > 0 and bonus != nil and salary != nil and salary > 0 do
  ...
  def check_if_failed(%Jugador{sueldo_completo: sueldo_comp})
      when sueldo_comp < 0 or sueldo_comp == nil do
    true
  end
  ...
  def get_final_salary_players(raw_players) do
    players_with_min_goals =
      Enum.map(raw_players, fn jugador ->
  ...
  def format_output_players(players_with_salary) do
    ...
    with %{true: failed, false: sucess} <- failed_separated do
      ...
      _err ->
        with %{true: failed} <- failed_separated do
          %{failed: failed}
        end
